...
var _ = Describe("Credential", func() {
	...
	Context("String representation", func() {
		It("should redact publicKey and correctly format fields", func() {
			id := uuid.New()
			externalID := []byte("external-id")
			userID := uuid.New()
			now := time.Now()
			const usbTransport = "usb"
			transports := []string{usbTransport}
			c, err := credential.New(id, externalID, userID, []byte("super-secret-pub-key"), transports, now)
			Expect(err).NotTo(HaveOccurred())

			expected := fmt.Sprintf("Credential{id: %v, externalID: %x, userID: %v, publicKey: ***REDACTED***, signCount: 0, transports: %v, createdAt: %v}", id, externalID, userID, transports, now)
