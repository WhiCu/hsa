...
## 2026-08-28 - Testcontainers overlayfs mount issue
**Finding:** Running the full test suite with `-tags=testcontainers` results in a panic inside `internal/infrastructure/storage/storage_test.go` specifically complaining about mounting overlayfs for the Postgres testcontainer when inside a container without correct root/docker nesting setups.
**Learning:** Tests spinning up containers locally via Testcontainers can fail heavily on environments using overlay filesystems inside dockerd if the docker-in-docker wrapper fails to support complex volume layering.
**Prevention/Action:** Be aware that even if docker is available locally, the testcontainers backend might fail due to environmental constraints. Always run fallback `go test ./...` without tags to isolate testcontainer test failures if needed.
